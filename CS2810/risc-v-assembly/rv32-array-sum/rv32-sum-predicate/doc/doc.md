...
Your function should implement this interface:
    ...
    int array_count(int *array, int count)

(all `int` values mean 32-bit integers)

`array` is the address of an array of 32-bit integers, and
`count` is the number of entries in the array (so the total memory
used by the array is `4 × count`).
...
1:
if not predicate(array[i]): goto 3
sum += array[i]
...
Because `array_count` calls another function, you must set up a
